...
std::string
ExtendibleHashtable::hash_function(const std::string &page_id) const {
...
void ExtendibleHashtable::insert_page(Page *page) {
  ...
  if (bucket->get_size() > this->bucket_max_size &&
      bucket->get_local_depth() < this->global_depth) {
...
void ExtendibleHashtable::split_bucket(const std::string &bucket_id) {
  ...
  std::string new_bucket_id = ((int)bucket_id.length() < this->global_depth)
                                  ? "1" + bucket_id
                                  : "1" + bucket_id.substr(1);
