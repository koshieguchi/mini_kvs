...
std::vector<std::pair<uint32_t, uint32_t>> KVStore::scan(uint32_t start_key,
                                                         uint32_t end_key) {
...
void KVStore::scan_memtable(
    Node *node, uint32_t start_key, uint32_t end_key,
    std::vector<std::pair<uint32_t, uint32_t>> *result) {
...
void KVStore::scan_ssts(uint32_t start_key, uint32_t end_key,
                        std::vector<std::pair<uint32_t, uint32_t>> *result) {
...
uint32_t KVStore::find_value_in_ssts(uint32_t key) {
  for (auto &level : levels) {
    for (auto &sst_path : level.sst_list) {
      uint32_t value =
          BTreeNode::search_value_by_key(key, sst_path, &buffer_pool);
      if (value == Utils::TOMB_STONE) {
        return Utils::INVALID_VALUE;  // Key does not exist since it has been
                                      // deleted.
      } else if (value != Utils::INVALID_VALUE) {
...
void KVStore::write_memtable_to_sst() {
  ...
  BTreeNode::construct_internal_nodes_and_write_to_file(leaf_nodes,
                                                        std::move(file));
  file.close();
