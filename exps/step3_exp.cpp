...
void benchmark_kvstore_put(int memtable_size, int put_item_size, std::ofstream& file, bool display_output) {
  ...
  if (!display_output){return;}

  file << "put," << (put_item_size/ExpConstants::ONE_MEGA_BYTE) << ","
            << num_items / (operation_duration.count() / (float)ExpConstants::TIME_CONVERSION_MICROSECONDS)
            << std::endl;
  kv.close();
}
...
void benchmark_kvstore_scan(int memtable_size, int put_item_size, std::ofstream& file) {
  ...
  const int num_scan_operations = Utils::PAGE_SIZE/Utils::ENTRY_SIZE; // Number of scan operations to perform (approx one page)
