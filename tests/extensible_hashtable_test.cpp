...
void test_initialization() {
  ExtendibleHashtable *hashtable =
      new ExtendibleHashtable(2, 4, 3);  // min_size, max_size, bucket_max_size
...
void test_get_num_directory() {
  ...
  assert(hashtable->get_num_directory() ==
         pow(2, hashtable->get_global_depth()));
...
void test_set_max_size() {
  ...
  assert(hashtable.get_global_depth() <=
         std::floor(std::log2(32)));  // Global depth should not exceed log2(32)
  ...
  assert(hashtable.get_num_directory() <=
         (size_t)(1 << hashtable.get_global_depth()));
...
void test_get_pair_bucket_id() {
  ...
  std::string pair_bucket_id =
      ExtendibleHashtable::get_pair_bucket_id(bucket_id);
